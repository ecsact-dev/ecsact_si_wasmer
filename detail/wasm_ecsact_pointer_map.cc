...
void* ecsactsi_wasm::as_host_pointer(std::int32_t guest_pointer_id) {
...
std::int32_t ecsactsi_wasm::as_guest_pointer(void* host_pointer) {
...
void ecsactsi_wasm::free_host_guest_pointer(void* host_pointer) {
	if(host_pointer != 0) {
		if(host_to_guest_ptr_map.contains(host_pointer)) {
